...
conversationStore.getPartenr(1, 10);
const getDataTable = computed(() => {
  return conversationStore.conversationPartner.rows;
});
const dataTable = reactive(getDataTable);

const getAppPage = computed(() => {
  return conversationStore.conversationPartner.max_page;
});
...
watch(allPages, () => {
  if (conversationStore.activeFilter) {
    ...
    conversationStore.getPartenr(selectPage.value, countItem.value);
  }
  const tmp = [];
