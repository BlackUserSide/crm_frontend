...
import { getDataTableOffer } from "./hard_code_data";
...
conversationStore.getGeo(1, 10);
const getDataTable = computed(() => {
  return conversationStore.conversationGeo.rows;
});
const dataTable = reactive(getDataTable);

const getAppPage = computed(() => {
  return conversationStore.conversationGeo.max_page;
});
...
watch(allPages, () => {
  if (conversationStore.activeFilter) {
    ...
    conversationStore.getGeo(selectPage.value, countItem.value);
  }
  const tmp = [];
  ...
  <div class="table-page table-page--second-conversation">
    ...
    <div class="table-page__desktop-table desktop-table">
      <div class="desktop-table__row desktop-table__row--thead">
        <div class="desktop-table__td">Country</div>
