...
function handlerSetActive(item: any) {
  ...
  props.dataOptions.value = props.dataOptions.sort((a: any, b: any) => {
    if (a.nameLanding === activeSelect.value.nameLanding) return -1;
    else return 1;
  });
  ...
  <div class="select" :class="activeSelectHandler && 'select--focus'">
    <div class="select__body">
      ...
      >
        <input
          class="input"
          v-model="activeSelect.nameLanding"
          type="text"
          disabled
          placeholder="Black"
        />
      ...
      >
        ...
        <div
          ...
          :class="
            activeSelect.nameLanding === item.nameLanding &&
            'select__option--focus'
          "
          @click="handlerSetActive(item)"
        >
          <span>{{ item.nameLanding }}</span>
        </div>
      </div>
    </div>
  </div>
